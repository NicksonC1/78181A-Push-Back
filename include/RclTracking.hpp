...
public:
    ...
    void updatePose(const genesis::Pose& botPose);
    bool isValid(double distVal) const;
    std::pair<CoordType, double> getBotCoord(const genesis::Pose& botPose, double accum = NAN);
...
class RclTracking {
public:
    RclTracking(genesis::Chassis* chassis_,
                int frequencyHz_ = 25,
                bool autoSync_ = true,
    ...
    // Accessors
    genesis::Pose getRclPose() const;
    void setRclPose(const genesis::Pose& p);
    void updateBotPose();
...
private:
    ...
    genesis::Pose latestPrecise, poseAtLatest;
