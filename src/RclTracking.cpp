...
void RclSensor::updatePose(const genesis::Pose& botPose) {
...
std::pair<CoordType, double> RclSensor::getBotCoord(const genesis::Pose& botPose, double accum) {
...
// Main RCL Tracking
RclTracking::RclTracking(genesis::Chassis* chassis_,
            int frequencyHz_,
            bool autoSync_,
            double minDelta_,
            double maxDelta_,
...
// Accessors
genesis::Pose RclTracking::getRclPose() const {
...
void RclTracking::setRclPose(const genesis::Pose& p) {
    latestPrecise = p;
...
void RclTracking::updateBotPose(RclSensor* sens) {
    if (sens != nullptr && chassis != nullptr) {
        ...
        // Sync to Lemlib
        chassis->setPose(genesis::Pose{static_cast<float>(pose.x), static_cast<float>(pose.y), static_cast<float>(chassis->getPose().theta)});
        setRclPose(genesis::Pose{static_cast<float>(pose.x), static_cast<float>(pose.y), static_cast<float>(chassis->getPose().theta)});
    }
}
...
void RclTracking::syncUpdate() {
    ...
    genesis::Pose currRclPosition = getRclPose();
